...
def on_this_day(month: int, day: int):
    ...
    try:
        ...
        date = datetime(year=2023, month=month, day=day).strftime(f"%d{get_ordinal_suffix(day)} of %B")
...
def main():
    parser = argparse.ArgumentParser(description='Get the Wikipedia "On this day.." events.')
